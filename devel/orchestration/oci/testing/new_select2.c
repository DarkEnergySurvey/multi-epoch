...
char *argv[];
{
  ...
  {
    printf(
        "%s <image list> -detector <detector (DECam or Mosaic2)> -catalog <output usnob.cat> -nite <nite> -quiet\n",
        argv[0]);
    exit(0);
  }
  ...
  sprintf(
      stmt,
      (text *)"SELECT chipid,raoffset,decoffset,rahwidth,dechwidth FROM wcsoffset WHERE TELESCOPE='%s' and DETECTOR='%s' ORDER BY chipid\n",
  ...
  while ( (retval = fetch( &ctx, &rs1)) == OCI_SUCCESS || (retval
          == OCI_SUCCESS_WITH_INFO))
  {
    ...
    sscanf(tmp2, "%d %f %f %f %f", &ccd[j], &raoff[j], &decoff[j], &rahw[j],
        &dechw[j]);
    j++;
    tmp2[0] = '\0';
  }
  ...
  sprintf(
      stmt,
      (text *)"SELECT distinct ra,dec FROM files WHERE (upper(imagetype)='OBJECT' OR imagetype='reduced') AND NITE='%s' ",
      nite);
          ...
          == OCI_SUCCESS_WITH_INFO))
  {
    ...
    for (i=1; i<=ccdtotal; i++)
    {
      ...
      printf( "ralow: %f rahigh: %f declow: %f dechigh: %f\n", ralow, rahigh,declow, dechigh );
