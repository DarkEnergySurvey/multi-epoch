...
QUERY_CCDS_Y2N = ''' 
     ...
     WHERE
         ...
         ops_proctag.TAG = '{tagname}' {and_extras} 
     '''
...
# Using the pre-computed felipe.ME_INPUTS_<TAGNAME> table
QUERY_ME_INPUTS = """
     SELECT
         {select_extras}
...
def get_CCDS_from_db_distance_old(dbh, tile_geometry, **kwargs): 
    """
    Execute the database query that returns the ccds and store them in a numpy record array
    kwargs: exec_name, tagname, select_extras, and_extras, from_extras
    ...
    if kwargs.get('tagname') == 'Y2T_FIRSTCUT' or kwargs.get('tagname') == 'Y2N_FIRSTCUT':
        QUERY_CCDS = QUERY_CCDS_Y2N
    else:
        QUERY_CCDS = QUERY_CCDS_Y2A1_indx
