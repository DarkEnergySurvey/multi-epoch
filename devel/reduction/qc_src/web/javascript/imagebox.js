...
//			jQuery('#ImageBoxNextImage').get(0).onclick = null;
	...
	loadImage : function(imageSrc, captiontext, pageSize, totalImages, iteration) //prevImage, nextImage)
	{
		...
		if (totalImages){
			if( jQuery.ImageBox.options.showCaption ){
		...
		clientSize = jQuery.iUtil.getClient();
		outerContainer
			.css('top', pageSize.t + jQuery.ImageBox.options.imageTop + 'px');
		...
		jQuery(imageEl)
			...
			function()
			{
				...
				containerH = 20 + imageEl.height + jQuery.ImageBox.options.border * 2;
				loader.hide();
				container.animate(
					{
						height		: containerH
