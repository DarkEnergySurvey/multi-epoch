...
def run():
	...
	if len(ra) > 0 and len(dec) > 0 and len(fwhm) > 0:
		...
		io.addOutput(plotName,'scaled_stars_plot','plot','Scaled Distribution','Spatial distribution of stars scaled by seeing.','file')
	else:
	...
	if len(ra) > 0 and len(dec) > 0 and len(xrms) > 0 and len(yrms) > 0:
		...
		io.addOutput(plotName,'psf_distortion_plot','plot','Distortion Distribution','Distribution of PSF distortion.','file')
	else:
	...
	if len(fwhm) > 0:
		...
		io.addOutput(plotName,'psf_histogram','plot','FWHM Histogram.','FWHM distribution.','file')
	else:
	...
	if len(major) > 0 and len(minor) and len(pa) > 0:
		...
		io.addOutput(plotName,'ellipticity_plot','plot','Ellipticity','Ellipticity of point sources.','file')
	else:
