...
/*------ Update header with the latest astrometric and photometric info */
		...
		"SCAMP astrometric dispersion RMS (internal)");
          fitswrite(tab->headbuf, str,&fgroup->sig_interr_hsn[d],
		H_EXPO, T_DOUBLE);
          sprintf(str, "ASTRRMS%1d", d+1);
          addkeywordto_head(tab, str,
		"SCAMP astrometric dispersion RMS (reference)");
          fitswrite(tab->headbuf, str, &fgroup->sig_referr_hsn[d],
		H_EXPO, T_DOUBLE);
          }
		...
		"SCAMP magnitude dispersion RMS (internal)");
