...
*	Author:		E.BERTIN (IAP)
*
*	Contents:	Compute the "global" photometric solution.
*
*	Last modify:	23/05/2007
*
*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
*/
...
/*---------------- Now fill the matrices */
			...
			|| (samp3->flags & (OBJ_SATUR|OBJ_TRUNC)))
                      continue;
                    set3 = samp3->set;
                    field3 = set3->field;
...
void	photstats_fgroup(fgroupstruct *fgroup, int instru, double hsn_thresh)
  {
   ...
   int		f,n,s, nfield,nsamp, nsource,nsource_hsn,
		ndeg,ndeg_hsn, nmean,nmean_hsn, nmatch,nmatch_hsn,
		nmeanr;

  lsig = lsig_hsn = 0.0;
  lchi2 = lchi2_hsn = 0.0;
  sn2 = 0.0;
  nsource = nsource_hsn = ndeg = ndeg_hsn = nmatch = nmatch_hsn = 0;
...
/* Now the stats with respect to the photometric reference fields */
  lsig = lsig_hsn = 0.0;
  lchi2 = lchi2_hsn = 0.0;
  ndeg = ndeg_hsn = nmatch = nmatch_hsn = 0;
  for (f=0; f<nfield; f++)
    {
    field=fields[f];
    for (s=0; s<field->nset; s++)
