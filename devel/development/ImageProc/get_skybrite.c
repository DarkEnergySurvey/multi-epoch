     ...
     char *argv[];
{
  ...
  float *indat,intemp,max,min,sig,mean,sum,binsize;
  FILE  *fin,*fout,*smscript,*pip;
  void  getband(),select_dblogin();
  
  if (argc<2) {
    printf("Usage: get_skybrite -nite <nite> -runid <runid>\n");
    printf("                OPTION:\n");
  ...
  for (i=1;i<argc;i++) {
    ...
    if (!strcmp(argv[i],"-version")) {
      printf("get_skybrite Version %2.2f\n",VERSION);
      exit(0);
    }
  ...
  for(j=bandstart;j<=bandstop;j++) {
    ...
    fprintf(fout,"select SKYBRITE from files where nite='%s' and runiddesc like '%s%%' and imagetype='%s' ",nite,runid,imtype);
    ...
    /* set up sql call */
    sprintf(sqlcall,"sqlplus -S %s < skybrite.sql > skybrite_%s.dat",dblogin,band);
    system(sqlcall);
  ...
  for(j=bandstart;j<=bandstop;j++) {
    ...
    sprintf(command,"wc -l skybrite_%s.dat",band);
    pip=popen(command,"r");
  ...
  for(j=bandstart;j<=bandstop;j++) {
    ...
    sprintf(fname,"skybrite_%s.dat",band);
    fin=fopen(fname,"r");
    while(fscanf(fin,"%f",&intemp)!=EOF) {
      indat[k]=log10(intemp);
      sum+=indat[k];
      k++;
    }
    fclose(fin);
  ...
  fprintf(smscript,"pt\n");
  fprintf(smscript,"\tdevice postencap %s_%s_skybrite.eps\n",nite,runid);
  k=0;
  for(j=bandstop;j>=bandstart;j--) {
    ...
    fprintf(smscript,"\tdata skybrite_%s.dat\n",band);
    fprintf(smscript,"\tread \{x 1\}\n");
    fprintf(smscript,"\tset lx=lg(x)\n");
    fprintf(smscript,"\tget_hist lx xx yy %2.3f %2.3f %2.5f\n",min,max,binsize);
    fprintf(smscript,"\tlimits %2.3f %2.3f yy\n",min-(0.03*min),max+(0.03*max));
    fprintf(smscript,"\tticksize -1 10 0 0\n");
    fprintf(smscript,"\thistogram xx yy\n");
    fprintf(smscript,"\tylabel N(%s)\n",band);
    if(j==bandstop) {
      fprintf(smscript,"\txlabel SKYBRITE\n");
  ...
  system("rm skybrite.sql skybrite_*.dat plot.sm tmp");
}  
