     ...
     char *argv[];
{
  ...
  if (argc<2) {
    printf("Usage: get_ellipticity -nite <nite> -runid <runid>\n");
    printf("                OPTION:\n");
  ...
  for (i=1;i<argc;i++) {
    ...
    if (!strcmp(argv[i],"-version")) {
      printf("get_ellipticity Version %2.2f\n",VERSION);
      exit(0);
    }
  ...
  for(j=bandstart;j<=bandstop;j++) {
    ...
    fprintf(fout,"select MD_ELLIPTICITY from files where nite='%s' and runiddesc like '%s%%' and imagetype='%s' ",nite,runid,imtype);
    ...
    /* set up sql call */
    sprintf(sqlcall,"sqlplus -S %s < ellipticity.sql > ellipticity_%s.dat",dblogin,band);
    system(sqlcall);
  ...
  for(j=bandstart;j<=bandstop;j++) {
    ...
    sprintf(command,"wc -l ellipticity_%s.dat",band);
    pip=popen(command,"r");
  ...
  for(j=bandstart;j<=bandstop;j++) {
    ...
    sprintf(fname,"ellipticity_%s.dat",band);
    fin=fopen(fname,"r");
  ...
  fprintf(smscript,"pt\n");
  fprintf(smscript,"\tdevice postencap %s_%s_ellipticity.eps\n",nite,runid);
  k=0;
  for(j=bandstop;j>=bandstart;j--) {
    ...
    fprintf(smscript,"\tdata ellipticity_%s.dat\n",band);
    ...
    if(j==bandstop) {
      fprintf(smscript,"\txlabel ellipticity\n");
  ...
  system("rm ellipticity.sql ellipticity_*.dat plot.sm tmp");
}  
