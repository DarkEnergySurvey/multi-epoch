    ...
    Matrix<double> *cov)
{
  ...
  while (!done) {
    ...
    for(uint i=0;i<v.size();i++) if( (*use)[i]) { 
...
bool Equivalent(double chisq1,double chisq2, int n1, int n2,
  double equivprob)
{
...
double betacf(double a,double b,double x)
{
  ...
  if (m>MAXIT) f2d_error("a or b too big in betai, or MAXIT too small");
  return h;
}
