...
template <class T>
class Legendre2D : public virtual Function2D<T> {
  ...
  private:
	...
	//temp[i] = ((2.*i-1.)*newxy*temp[i-1] - (i-1.)*temp[i-2])/i;
      if (order > 1) {
	tmv::Vector<double>::iterator vi_m2 = temp.begin();
	tmv::Vector<double>::iterator vi_m1 = temp.begin()+1;
	tmv::Vector<double>::iterator vi = temp.begin()+2;
	double twoim1 = 3.;
	double im1 = 1.;
	for(int i=2;i<=order;i++,twoim1+=2.,im1++,vi_m2++,vi_m1++,vi++)
	  *vi = (twoim1*newxy*(*vi_m1) - im1*(*vi_m2))/double(i);
      }
      //xdbg<<"temp = "<<temp<<endl;
      return temp;
    }
