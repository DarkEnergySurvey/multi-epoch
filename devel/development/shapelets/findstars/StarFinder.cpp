        ...
        vector<float>& mag)
{
  ...
  for (unsigned long i=0; i<flags.size(); i++)
  {
    ...
    maybestars.push_back(
            new PotentialStar(Position(x[i],y[i]),mag[i],sigma[i],i,""));

  }

  std::cout<<"  Possible Stars: "<<count<<"/"<<flags.size()<<"\n";

  cout<<"  Running FindStars\n";
  vector<PotentialStar*> stars = FindStars(maybestars);

  cout<<"  Found "<<stars.size()<<"\n";
  vector<long> idlist;
  for (unsigned long i=0; i<stars.size();i++)
    idlist.push_back( stars[i]->GetIndex() );
  return(idlist);
}
...
{
  cout<<"Loading StarFinder config"<<mCFile<<endl;
  ...
  try
  {
    mMinsize = mConfigFile["minsize"];
    mMaxsize = mConfigFile["maxsize"];
    mMinmag = mConfigFile["minmag"];
    mMaxmag = mConfigFile["maxmag"];
    mMaxoutmag = mConfigFile["maxoutmag"];


    mNdivx = mConfigFile["ndivx"];
    mNdivy = mConfigFile["ndivy"];

    mStartn1 = mConfigFile["startn1"];
    mStarfrac = mConfigFile["starfrac"];
    mMagstep1 = mConfigFile["magstep1"];
    mReject1 = mConfigFile["reject1"];
    mMaxratio1 = mConfigFile["maxratio1"];
    mBinsize1 = mConfigFile["binsize1"];

    mMiniter1 = mConfigFile["miniter1"];
    mMaxrms = mConfigFile["maxrms"];

    mStartn2 = mConfigFile["startn2"];
    mMagstep2 = mConfigFile["magstep2"];
    mMinbinsize = mConfigFile["minbinsize"];
    mReject2 = mConfigFile["reject2"];
    mPurityratio = mConfigFile["purityratio"];
    mMiniter2 = mConfigFile["miniter2"];

    mStarsperbin = mConfigFile["starsperbin"];
    mFitorder = mConfigFile["fitorder"];
    mFitsigclip = mConfigFile["fitsigclip"];
    mOkvalcount = mConfigFile["okvalcount"];

    mMaxrefititer = mConfigFile["maxrefititer"];
  }
  catch (string s)
  {
...
std::vector<string> StarFinder::RequiredConfigFields()
{
...
vector<PotentialStar*> StarFinder::FindStars(vector<PotentialStar*>& allobj)
{
...
// Use these stars to fit the variation in rsq across the image.
  ...
  vector<vector<Bounds> > qbounds = totalbounds.Divide(mNdivx,mNdivy);
  xdbg<<"made qbounds\n";
...
// This time step by 0.25 mag (mMagstep2).
  ...
  for(size_t iter=0;refit && iter<mMaxrefititer;iter++) {
    dbg<<"starting refit\n"<<endl;
    refit = false;
    vector<vector<vector<PotentialStar*> > > starsarray(mNdivx,
	vector<vector<PotentialStar*> >(mNdivy));
...
void StarFinder::FindMinMax(const vector<PotentialStar*>& list, 
...
void StarFinder::OutlierReject(vector<PotentialStar*>& list, 
...
StarFinder::GetPeakList(const vector<PotentialStar*>& objlist,
		...
		const vector<PotentialStar*>& starlist, double *outsigma)
{
  ...
  if (starlist.size() <= (order+1)*(order+2)/2) {
	...
	"Increase mStarsperbin or decrease mFitorder\n",
	ALGORITHMERROR);
  }
...
void StarFinder::RoughlyFitBrightStars(const vector<PotentialStar*>& objlist,
    Function2D<double> *f,double *outsigma)
{
  ...
  if (2*five-1 >= brightlist.size()) {
    ErrorExit(
	"Too few objects in brightlist.  Increase mStartn1.\n",
	PARAMETERERROR);
  }
