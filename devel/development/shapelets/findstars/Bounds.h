...
class Position : public std::complex<double> {

public:
    Position() : std::complex<double>() {}
    Position(const Position& rhs) : std::complex<double>(rhs) {}
    Position(const std::complex<double>& rhs) : std::complex<double>(rhs) {}
    ~Position() {}
    Position(double xin,double yin) : std::complex<double>(xin,yin) {}
    Position& operator=(const Position& rhs) 
    { std::complex<double>::operator=(rhs); return *this; }
    Position& operator=(const std::complex<double>& rhs) 
    { std::complex<double>::operator=(rhs); return *this; }

    double GetX() const {return(this->real());}
    double GetY() const {return(this->imag());}

    void Read(std::istream& fin) 
    { double x,y; fin >> x >> y; *this = std::complex<double>(x,y); }
...
public:
    ...
    { if (!b2.IsDefined()) return true;
      else return (defined && b2.xmin >= xmin && b2.xmax <= xmax &&
                   b2.ymin >= ymin && b2.ymax <= ymax); }
    double Area() const
    { return defined ? (xmax-xmin)*(ymax-ymin) : 0.; }
    std::vector<std::vector<Bounds> > Quarter() const 
    { return Divide(2,2); }
    std::vector<std::vector<Bounds> > Divide(uint nx, uint ny) const;
    void Write(std::ostream& fout) const
